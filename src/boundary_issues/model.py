...
class ConvPass(torch.nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_sizes, activation, padding=("same", "valid", "valid")
    ):
        ...
        for kernel_size in kernel_sizes:
            ...
            # Calculate padding for each dimension based on padding_modes
            for k, mode in zip(kernel_size, padding):
                if mode == "same":
                    pad.append(k // 2)  # "same" padding
                elif mode == "valid":
                    pad.append(0)  # "valid" padding
                else:
                    raise ValueError(f"Unsupported padding mode: {mode}")

            # Convert pad list to tuple
            pad = tuple(pad)
